#!/usr/bin/env python3
"""
TTS Workaround Fixed - Handles XTTS speaker requirements properly
"""

import torch
import sys
import os
import traceback

# Patch torch.load to use weights_only=False
original_load = torch.load

def patched_load(*args, **kwargs):
    kwargs['weights_only'] = False
    return original_load(*args, **kwargs)

torch.load = patched_load

# Now import TTS after patching
from TTS.api import TTS
from flask import Flask, request, jsonify, send_file
import tempfile
import uuid
import numpy as np

app = Flask(__name__)

# Initialize TTS model
print("Loading TTS model...")
tts = None
model_type = None

try:
    tts = TTS("tts_models/multilingual/multi-dataset/xtts_v2")
    print("XTTS v2 model loaded successfully!")
    model_type = "xtts"
    
    # Get available speakers for XTTS
    if hasattr(tts.synthesizer.tts_model, 'speaker_manager') and tts.synthesizer.tts_model.speaker_manager:
        speakers = list(tts.synthesizer.tts_model.speaker_manager.name_to_id.keys())
        print(f"Available speakers: {speakers}")
        default_speaker = speakers[0] if speakers else None
    else:
        default_speaker = None
        print("No speaker manager found, will use voice cloning")
        
except Exception as e:
    print(f"Failed to load XTTS v2: {e}")
    try:
        tts = TTS("tts_models/en/ljspeech/vits")
        print("VITS model loaded successfully!")
        model_type = "vits"
        default_speaker = None
    except Exception as e2:
        print(f"Failed to load VITS model: {e2}")
        sys.exit(1)

@app.route('/tts', methods=['POST'])
def synthesize():
    try:
        data = request.get_json()
        text = data.get('text', '')
        language = data.get('language', 'en')
        
        if not text:
            return jsonify({'error': 'No text provided'}), 400
        
        print(f"Synthesizing: {text} (lang: {language})")
        
        # Generate audio
        temp_file = f"/tmp/tts_{uuid.uuid4()}.wav"
        
        if model_type == "xtts":
            # For XTTS, we need to handle speaker reference
            try:
                if default_speaker:
                    # Use predefined speaker
                    wav = tts.tts(text=text, speaker=default_speaker, language=language)
                else:
                    # Use voice cloning with a reference audio (create a simple one)
                    # For now, let's try without speaker first and see if it works
                    wav = tts.tts(text=text, language=language)
                
                # Save the audio
                import soundfile as sf
                if isinstance(wav, list):
                    wav = np.array(wav)
                
                # Ensure we have a valid audio array
                if len(wav) > 0:
                    sf.write(temp_file, wav, 22050)
                else:
                    raise Exception("Generated audio is empty")
                    
            except Exception as xtts_error:
                print(f"XTTS synthesis failed: {xtts_error}")
                # Fallback: try to use the model differently
                try:
                    # Try with speaker_wav (voice cloning approach)
                    # We'll use the existing habari.wav as reference if it exists
                    ref_audio = "/home/kouragita/tractor-dev-ai/tts_coqui/habari.wav"
                    if os.path.exists(ref_audio):
                        wav = tts.tts(text=text, speaker_wav=ref_audio, language=language)
                        import soundfile as sf
                        if isinstance(wav, list):
                            wav = np.array(wav)
                        sf.write(temp_file, wav, 22050)
                    else:
                        raise Exception("No reference audio available for voice cloning")
                except Exception as fallback_error:
                    print(f"Fallback also failed: {fallback_error}")
                    return jsonify({'error': f'TTS synthesis failed: {str(xtts_error)}'}), 500
        else:
            # For VITS and other models
            tts.tts_to_file(text=text, file_path=temp_file)
        
        print(f"Audio saved to: {temp_file}")
        
        if os.path.exists(temp_file) and os.path.getsize(temp_file) > 1000:  # Check if file has content
            return send_file(temp_file, as_attachment=True, download_name='output.wav')
        else:
            return jsonify({'error': 'Generated audio file is empty or too small'}), 500
        
    except Exception as e:
        print(f"Error in synthesis: {e}")
        traceback.print_exc()
        return jsonify({'error': str(e)}), 500

@app.route('/health', methods=['GET'])
def health():
    return jsonify({
        'status': 'healthy', 
        'model_type': model_type,
        'default_speaker': default_speaker if model_type == "xtts" else None,
        'model': str(type(tts.synthesizer.tts_model)) if tts else 'None'
    })

@app.route('/', methods=['GET'])
def root():
    return jsonify({
        'service': 'TTS Workaround Server (Fixed)',
        'model_type': model_type,
        'endpoints': {
            '/tts': 'POST - Synthesize text to speech (JSON: {"text": "...", "language": "en"})',
            '/health': 'GET - Health check'
        }
    })

if __name__ == '__main__':
    port = int(os.environ.get('PORT', 8082))
    print(f"Starting TTS Workaround Server (Fixed) on port {port}")
    app.run(host='0.0.0.0', port=port, debug=False)
